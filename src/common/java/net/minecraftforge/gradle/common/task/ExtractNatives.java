...
public class ExtractNatives extends DefaultTask {
    private final RegularFileProperty meta;
    private final DirectoryProperty output;

    public ExtractNatives() {
        this.meta = getProject().getObjects().fileProperty();
        this.output = getProject().getObjects().directoryProperty();
    }

    @TaskAction
    public void run() throws IOException {
        VersionJson json = Utils.loadJson(meta.get().getAsFile(), VersionJson.class);
        for (LibraryDownload lib : json.getNatives()) {
            ...
            Utils.extractZip(target, output.get().getAsFile(), false);
        }
    }

    @InputFile
    public RegularFileProperty getMeta() {
        return this.meta;
    }
